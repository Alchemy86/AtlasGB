...
**359 entries** · 159 distinct addresses · **141 with a written description** · 147 repeated slots folded onto slot 1.

| address | bytes | symbol | ev | what it is |
|---|---:|---|:--:|---|
| `$CC2D` | 1 | <a id="s-wBattleAndStartSavedMenuItem"></a>`wBattleAndStartSavedMenuItem` | RL | Measured in the same gym battle's own intro/continue sequence, before any menu was opened by this run's script: changes once, from 4 to 0, at frame 49 -- too little data this round to say more than that a write happens this early. |
...
| `$CCD5` | 2 | <a id="s-wAILayer2Encouragement"></a>`wAILayer2Encouragement` | RL | Increments by 1 each time the enemy AI's move-selection scratch (wEnemyMoveNum and its siblings) briefly reloads a different move before settling. Measured three times in the same gym battle, 0 to 1 to 2 to 3, exactly at those three reconsideration events. The low byte never changed. |
...
| `$CCE3` | 2 | <a id="s-wLastSwitchInEnemyMonHP"></a>`wLastSwitchInEnemyMonHP` | RL | The opponent's HP at the moment it was sent out -- a snapshot kept alongside the live, changing wEnemyMonHP. Measured in the same gym battle: $0029 (41, matching STARYU's own HP) loads the instant it switches in; the high byte never changed across the whole battle. |
...
| `$CCF0` | 1 | <a id="s-wInHandlePlayerMonFainted"></a>`wInHandlePlayerMonFainted` | RL | A flag set once the active Pokemon's HP reaches 0. Measured in the same gym battle: 0 throughout, then 1 exactly 27 frames after wBattleMonHP first reads 0 -- the gap between the HP hitting zero and this flag being noticed is itself real, measured behavior, not simultaneity. |
| `$CCF1` | 1 | <a id="s-wPlayerUsedMove"></a>`wPlayerUsedMove` | RL | The move ID actually executed this turn, set later than wPlayerMoveNum (which loads as soon as a move is selected in the menu, well before the turn resolves). Measured in the same gym battle: set to 33 partway through the turn that first damaged STARYU, roughly 375 frames after wPlayerMoveNum had already loaded 33 for that same turn. *(related: [`wDamageMultipliers`](battle.md#s-wDamageMultipliers), [`wEnemyUsedMove`](battle.md#s-wEnemyUsedMove), [`wPlayerBattleStatus1`](battle.md#s-wPlayerBattleStatus1), [`wPlayerBattleStatus2`](battle.md#s-wPlayerBattleStatus2), [`wPlayerBattleStatus3`](battle.md#s-wPlayerBattleStatus3), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve))* |
| `$CCF2` | 1 | <a id="s-wEnemyUsedMove"></a>`wEnemyUsedMove` | RL | Same role as wPlayerUsedMove, for the opponent's move. Measured in the same gym battle: set to the opponent's move ID during each turn's resolution (55, then later 33) and reset to 0 before the next turn's menu opens -- unlike wEnemyMoveNum, which reloads straight to the new move rather than resetting to 0 first. *(related: [`wDamageMultipliers`](battle.md#s-wDamageMultipliers), [`wPlayerBattleStatus1`](battle.md#s-wPlayerBattleStatus1), [`wPlayerBattleStatus2`](battle.md#s-wPlayerBattleStatus2), [`wPlayerBattleStatus3`](battle.md#s-wPlayerBattleStatus3), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve), [`wPlayerUsedMove`](battle.md#s-wPlayerUsedMove))* |
| `$CCF3` | 1 | <a id="s-wEnemyMonMinimized"></a>`wEnemyMonMinimized` | RL | The same for the opponent. |
| `$CCF4` | 1 | <a id="s-wMoveDidntMiss"></a>`wMoveDidntMiss` | RL | A flag that toggles 0 to 1 and back to 0 within each battle turn -- five times total across this gym battle, one per completed turn, never holding 1 across a turn boundary. The per-turn toggle and the name both point at "this turn's accuracy check passed" (compare wMoveMissed, the confirmed accuracy-check-failed flag elsewhere in this atlas), but this trace does not pin down the exact instant it flips relative to the turn's damage calculation. |
...
| `$CD0F` | 1 | <a id="s-wPlayerMonUnmodifiedLevel"></a>`wPlayerMonUnmodifiedLevel`<br><a id="s-wInGameTradeGiveMonSpecies"></a>`wInGameTradeGiveMonSpecies` <a id="s-wMiscBattleDataEnd"></a>`wMiscBattleDataEnd` <a id="s-wVermilionDockTileMapBufferEnd"></a>`wVermilionDockTileMapBufferEnd` | RL | The active party member's level before any in-battle modifier, refreshed each time a new Pokemon is sent out. Measured in the same gym battle: 12 for the first party member sent out, 5 for the second, both loading the same frame wBattleMonHP populates for that Pokemon. |
...
| `$CD1A` | 1 | <a id="s-wPlayerMonAttackMod"></a>`wPlayerMonAttackMod`<br><a id="s-wPlayerMonStatMods"></a>`wPlayerMonStatMods` | RL | **`wPlayerMonAttackMod`** — Attack stage for your side, **biased by 7**: `1` to `13` is `-6` to `+6`, and a freshly zeroed block reads as `-6` on everything until `InitBattleVariables` writes the sevens. **`wPlayerMonStatMods`** — In-battle stat stages for your side. **7 is neutral, not 0**: the block is zeroed work RAM until `InitBattleVariables` writes the sevens, so a sample taken on the first frames of a battle reads zero everywhere and looks like -6 on everything. Measured: also written, together with `wPlayerMonDefenseMod`/`wPlayerMonSpeedMod`/`wPlayerMonSpecialMod`/`wPlayerMonEvasionMod`, from bank 15 `$4C0E` — a different routine from `InitBattleVariables` (bank 20 `$65AF`) that writes the same sevens, most likely the equivalent reset on a mid-battle switch-in rather than battle start; not confirmed further here. *(related: [`wInGameTradeReceiveMonName`](battle.md#s-wInGameTradeReceiveMonName), [`wPlayerMonDefenseMod`](battle.md#s-wPlayerMonDefenseMod), [`wPlayerMonEvasionMod`](battle.md#s-wPlayerMonEvasionMod), [`wPlayerMonSpecialMod`](battle.md#s-wPlayerMonSpecialMod), [`wPlayerMonSpeedMod`](battle.md#s-wPlayerMonSpeedMod))* |
| `$CD1B` | 1 | <a id="s-wPlayerMonDefenseMod"></a>`wPlayerMonDefenseMod` | L | Defense stage for your side, **biased by 7**: `1` to `13` is `-6` to `+6`, and a freshly zeroed block reads as `-6` on everything until `InitBattleVariables` writes the sevens. *(related: [`wInGameTradeReceiveMonName`](battle.md#s-wInGameTradeReceiveMonName), [`wPlayerMonEvasionMod`](battle.md#s-wPlayerMonEvasionMod), [`wPlayerMonSpecialMod`](battle.md#s-wPlayerMonSpecialMod), [`wPlayerMonSpeedMod`](battle.md#s-wPlayerMonSpeedMod), [`wPlayerMonStatMods`](battle.md#s-wPlayerMonStatMods))* |
| `$CD1C` | 1 | <a id="s-wPlayerMonSpeedMod"></a>`wPlayerMonSpeedMod` | RL | Speed stage for your side, **biased by 7**: `1` to `13` is `-6` to `+6`, and a freshly zeroed block reads as `-6` on everything until `InitBattleVariables` writes the sevens. *(related: [`wInGameTradeReceiveMonName`](battle.md#s-wInGameTradeReceiveMonName), [`wPlayerMonDefenseMod`](battle.md#s-wPlayerMonDefenseMod), [`wPlayerMonEvasionMod`](battle.md#s-wPlayerMonEvasionMod), [`wPlayerMonSpecialMod`](battle.md#s-wPlayerMonSpecialMod), [`wPlayerMonStatMods`](battle.md#s-wPlayerMonStatMods))* |
| `$CD1D` | 1 | <a id="s-wPlayerMonSpecialMod"></a>`wPlayerMonSpecialMod` | RL | Special stage for your side, **biased by 7**: `1` to `13` is `-6` to `+6`, and a freshly zeroed block reads as `-6` on everything until `InitBattleVariables` writes the sevens. *(related: [`wInGameTradeReceiveMonName`](battle.md#s-wInGameTradeReceiveMonName), [`wPlayerMonDefenseMod`](battle.md#s-wPlayerMonDefenseMod), [`wPlayerMonEvasionMod`](battle.md#s-wPlayerMonEvasionMod), [`wPlayerMonSpeedMod`](battle.md#s-wPlayerMonSpeedMod), [`wPlayerMonStatMods`](battle.md#s-wPlayerMonStatMods))* |
| `$CD1E` | 1 | <a id="s-wPlayerMonAccuracyMod"></a>`wPlayerMonAccuracyMod`<br><a id="s-wInGameTradeReceiveMonName"></a>`wInGameTradeReceiveMonName` | R | Accuracy stage for your side, biased by 7. **There is no accuracy value anywhere** — only this stage: `MoveHitTest` scales the move table's accuracy byte by it and by the inverse of the defender's evasion stage, so the number in the move table stops being true the first time a SAND-ATTACK lands. Measured on the cartridge: TACKLE's 242/255 becomes 159/255 after one stage down and 121/255 after two. *(related: [`wPlayerMonDefenseMod`](battle.md#s-wPlayerMonDefenseMod), [`wPlayerMonEvasionMod`](battle.md#s-wPlayerMonEvasionMod), [`wPlayerMonSpecialMod`](battle.md#s-wPlayerMonSpecialMod), [`wPlayerMonSpeedMod`](battle.md#s-wPlayerMonSpeedMod), [`wPlayerMonStatMods`](battle.md#s-wPlayerMonStatMods))* |
| `$CD1F` | 3 | <a id="s-wPlayerMonEvasionMod"></a>`wPlayerMonEvasionMod` | RL | Evasion stage for your side, biased by 7, and the other half of `MoveHitTest`'s scaling. *(related: [`wInGameTradeReceiveMonName`](battle.md#s-wInGameTradeReceiveMonName), [`wPlayerMonDefenseMod`](battle.md#s-wPlayerMonDefenseMod), [`wPlayerMonSpecialMod`](battle.md#s-wPlayerMonSpecialMod), [`wPlayerMonSpeedMod`](battle.md#s-wPlayerMonSpeedMod), [`wPlayerMonStatMods`](battle.md#s-wPlayerMonStatMods))* |
| `$CD22` | 1 | <a id="s-wPlayerMonStatModsEnd"></a>`wPlayerMonStatModsEnd` | RL | One past your side's six stat stages. |
| `$CD23` | 1 | <a id="s-wEnemyMonUnmodifiedLevel"></a>`wEnemyMonUnmodifiedLevel` | RL | The opponent's level before any in-battle modifier is applied. Measured across a real gym battle against Misty, played to a black-out: loads to 18 at switch-in, matching STARYU's own documented level-18 HP (see discoveries.md's cerulean-gym.md coverage, wEnemyMonHP settling at 41). |
...
| `$CD2D` | 1 | <a id="s-wEngagedTrainerClass"></a>`wEngagedTrainerClass` | RL | A trainer-class value loaded at battle start, distinct from wTrainerClass. Measured in the same gym battle against Misty (wTrainerClass loads to 35): wEngagedTrainerClass loads to 33 -- a different number for the same fight, not explained further this round. |
...
| `$CD6D` | 4 | <a id="s-wBattleMenuCurrentPP"></a>`wBattleMenuCurrentPP`<br><a id="s-wEvoDataBuffer"></a>`wEvoDataBuffer` <a id="s-wMoveData"></a>`wMoveData` <a id="s-wNameBuffer"></a>`wNameBuffer` <a id="s-wPayDayMoney"></a>`wPayDayMoney` | RL | The four move slots' current-PP values shown on the FIGHT menu. Measured in the same gym battle: extremely noisy through most of a turn -- driven almost entirely by the same ROM0 $00B6 generic fill primitive already excluded from related elsewhere in this atlas, reused for many unrelated purposes between calls -- but briefly settles to the real value right after a move is chosen (matching wPlayerMoveMaxPP, e.g. 30 for the move loaded at frame 867). Intermediate values are not meaningful; only the settled one is. |
| `$CEEB` | 1 | <a id="s-wHPBarOldHP"></a>`wHPBarOldHP`<br><a id="s-wAlphabetCase"></a>`wAlphabetCase` <a id="s-wEvoMonTileOffset"></a>`wEvoMonTileOffset` | RL | The health bar's animation start point. Measured in the same gym battle: loads to the pre-hit HP the instant a hit lands, then ticks down by 1 roughly every 5-7 frames in step with wHPBarNewHP until they meet -- watched directly across both HP drops in this battle (STARYU 41->40->39, and the player's own Pokemon's two faints), the visible mechanism behind the animated health-bar wipe. |
| `$CEED` | 2 | <a id="s-wHPBarNewHP"></a>`wHPBarNewHP`<br><a id="s-wNamingScreenLetter"></a>`wNamingScreenLetter` | RL | The health bar's animation target, 2 bytes big-endian. Measured in the same gym battle: set to the post-hit HP the instant a hit lands, then wHPBarOldHP counts down to meet it a frame at a time -- see wHPBarOldHP. |
| `$CEEF` | 1 | <a id="s-wHPBarDelta"></a>`wHPBarDelta` | RL | A one-shot value written once and left alone afterward. Measured once in the same gym battle: set to 255 ($FF) at the moment the player's first Pokemon fainted, and never changed again for the rest of the battle, including STARYU's own HP drops and the second faint -- not enough data this round to say what triggers a write versus what leaves it alone. |
...
| `$CF07` | 1 | <a id="s-wAnimSoundID"></a>`wAnimSoundID` | RL | Which sound effect the current battle animation frame should play. Measured across a real playthrough (walk, a wild battle, a trainer battle, a forced faint, menus, a save): written from bank 30 `$414B`, during faint-battle, faint-setup, intro, party-reorder, post-save walk/menus, save, start-menu-probe, trainer-battle; values seen: [0, 22, 30, 44, 132, 255]. |
...
| `$D05B` | 1 | <a id="s-wDamageMultipliers"></a>`wDamageMultipliers` | RL | A scratch byte touched during each hit's damage calculation. Measured in the same gym battle: raw values 10 and 138 (not independently confirmed against a public encoding this round), one per hit, reset to 0 immediately before the next Pokemon is sent out -- in the same contiguous instruction sequence (bank 15, $4CB7 onward) that also clears wPlayerMoveNum, wPlayerUsedMove, wEnemyUsedMove and the wPlayerStatsToDouble..wPlayerBattleStatus3 group (see related). *(related: [`wEnemyUsedMove`](battle.md#s-wEnemyUsedMove), [`wPlayerBattleStatus1`](battle.md#s-wPlayerBattleStatus1), [`wPlayerBattleStatus2`](battle.md#s-wPlayerBattleStatus2), [`wPlayerBattleStatus3`](battle.md#s-wPlayerBattleStatus3), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve), [`wPlayerUsedMove`](battle.md#s-wPlayerUsedMove))* |
...
| `$D062` | 1 | <a id="s-wPlayerBattleStatus1"></a>`wPlayerBattleStatus1` | RL | Volatile battle state for your side: confusion, trapping, flinching and the rest, one bit each. *(related: [`wDamageMultipliers`](battle.md#s-wDamageMultipliers), [`wEnemyUsedMove`](battle.md#s-wEnemyUsedMove), [`wPlayerBattleStatus2`](battle.md#s-wPlayerBattleStatus2), [`wPlayerBattleStatus3`](battle.md#s-wPlayerBattleStatus3), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve), [`wPlayerUsedMove`](battle.md#s-wPlayerUsedMove))* *(×3, stride 1)* |
| `$D063` | 1 | <a id="s-wPlayerBattleStatus2"></a>`wPlayerBattleStatus2` | RL | The second volatile byte for your side. It carries `GETTING_PUMPED`, the FOCUS ENERGY bit, with no counter beside it — and in Generation 1 that bit **quarters** the critical-hit rate instead of raising it, because `CriticalHitTest` shifts the wrong way. Measured for a base-Speed-140 Electrode: 0.2604 of 960 attacks crit without it, 0.0612 of 588 with. *(related: [`wDamageMultipliers`](battle.md#s-wDamageMultipliers), [`wEnemyUsedMove`](battle.md#s-wEnemyUsedMove), [`wPlayerBattleStatus1`](battle.md#s-wPlayerBattleStatus1), [`wPlayerBattleStatus3`](battle.md#s-wPlayerBattleStatus3), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve), [`wPlayerUsedMove`](battle.md#s-wPlayerUsedMove))* |
| `$D064` | 1 | <a id="s-wPlayerBattleStatus3"></a>`wPlayerBattleStatus3` | RL | The third volatile byte for your side. Bit 1 is LIGHT SCREEN and bit 2 is REFLECT, and in Generation 1 **both belong to the Pokemon, not to the side**: `SendOutMon` zeroes this byte, so a screen does not survive a switch. Measured driving a real voluntary switch: `$04` with REFLECT up, `$00` immediately afterwards, with the opponent's copy at `$00` throughout. *(related: [`wDamageMultipliers`](battle.md#s-wDamageMultipliers), [`wEnemyUsedMove`](battle.md#s-wEnemyUsedMove), [`wPlayerBattleStatus1`](battle.md#s-wPlayerBattleStatus1), [`wPlayerBattleStatus2`](battle.md#s-wPlayerBattleStatus2), [`wPlayerMoveNum`](scratch.md#s-wPlayerMoveNum), [`wPlayerStatsToDouble`](scratch.md#s-wPlayerStatsToDouble), [`wPlayerStatsToHalve`](scratch.md#s-wPlayerStatsToHalve), [`wPlayerUsedMove`](battle.md#s-wPlayerUsedMove))* |
...
| `$D07C` | 1 | <a id="s-wAnimationID"></a>`wAnimationID`<br><a id="s-wDefaultMap"></a>`wDefaultMap` <a id="s-wMenuItemOffset"></a>`wMenuItemOffset` | RL | Which battle animation is currently playing. Measured across a real playthrough (walk, a wild battle, a trainer battle, a forced faint, menus, a save): written from bank 15 `$4292`, during faint-battle, faint-setup, intro, party-reorder, post-save walk/menus, save, start-menu-probe, trainer-battle; values seen: [0, 1, 23, 31, 39, 45, 193, 195]. |
| `$D081` | 1 | <a id="s-wBaseCoordX"></a>`wBaseCoordX` | RL | The screen column the current battle animation's effect is drawn from. Measured across a real playthrough (walk, a wild battle, a trainer battle, a forced faint, menus, a save): written from bank 30 `$4E90`, during faint-battle, intro, party-reorder, post-save walk/menus, save, start-menu-probe, trainer-battle; 22 distinct values, 0-152. |
| `$D082` | 1 | <a id="s-wBaseCoordY"></a>`wBaseCoordY` | RL | The screen row the current battle animation's effect is drawn from. Measured across a real playthrough (walk, a wild battle, a trainer battle, a forced faint, menus, a save): written from bank 30 `$4E8C`, during faint-battle, intro, party-reorder, post-save walk/menus, save, start-menu-probe, trainer-battle; 19 distinct values, 0-96. |
