...
**26 entries** · 16 distinct addresses · **11 with a written description**.
...
| `$CD81` | 360 | <a id="s-wSerialOtherGameboyRandomNumberListBlock"></a>`wSerialOtherGameboyRandomNumberListBlock`<br><a id="s-wTileMapBackup2"></a>`wTileMapBackup2` | RL | **`wSerialOtherGameboyRandomNumberListBlock`** — Shares this address with `wTileMapBackup2` — the link-battle shared random-number list borrows this scratch buffer outside a link session; see that entry for the rest of what it is used for. **`wTileMapBackup2`** — A second 360-byte screen backup. Also the largest clean scratch buffer below the save boundary, which is why other tools borrow it. |
